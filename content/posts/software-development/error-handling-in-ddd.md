...
Conversely, the application layer _does_ handle the user's request directly and therefore has full sight of the user's original context. As such, it can generate contextual error messages and map them back to specific fields in the user's request (if necessary).
...
1. Clean.
1. Unlike `Result` objects, exceptions _do_ bubble up the call stack and so don't have to be handled by each layer explicitly if not necessary, for example if they don't need to be translated into more contextual error messages than the default message generated by the domain layer.
...
An alternative approach is to have the domain layer return `Result` objects from each operation. If an operation results in an error, then the `Result` object contains some kind of error _object_ that the application layer can translate into a user-friendly error message, and map it back to a specific field in the user's request (if necessary). The subtype of the error object and the data contained therein fully describe the error, which allows the application layer to translate it accordingly.
...
1. Unlike exceptions, `Result` objects do _not_ bubble up the call stack and so have to be handled by each layer explicitly. _Some actually see this more explicit control-flow as a benefit._
